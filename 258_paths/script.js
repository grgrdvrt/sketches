...
class Line{
	...
	constructor(nPts, thickness, thicknessVariation, speed, color, initialSite){
	...
	draw(ctx){
		ctx.save();
		ctx.beginPath();
		ctx.lineWidth = this.thicknessVariation * this.thickness;
...
const thicknesses = [1, 0.75, 0.5, 0.25];
const lines = [];
const total = 20;
for(let i = 0; i < total; i++){
	lines[i] = new Line(
		// Math.floor(lerp(30, 250, Math.random())),
		100,
		margin,
		thicknesses[Math.floor(thicknesses.length * i / total)],
		0.15,
