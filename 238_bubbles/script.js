  ...
  for(let i = 0; i < n; i++){
    ...
    for(let j = 0; j < i; j++){
      ...
      if(d.getLength() < r1 + b2.radius){
        d.setLength((r1 + b2.radius) - (d.getLength())).multiplyScalar(0.5 * paddingRatio);
        b1.pos.add(d);
        b2.pos.sub(d);
      }
    }
